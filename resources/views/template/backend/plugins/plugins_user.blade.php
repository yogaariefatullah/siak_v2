    ...
    <div class="offcanvas-content pr-5 mr-n5">
        ...
        <div class="d-flex align-items-center mt-5">
            <div class="symbol symbol-100 mr-2">
                <div class="symbol-label" style="background-image:url({{asset('assets/logo.gif')}});background-size: 90%;"></div>
            ...
            <div class="d-flex flex-column">
                ...
                @if(Auth::guard('traders')->check())
                    <div class="text-muted mt-1">Perusahaan IUP OPK</div>
                @elseif(Auth::guard('surveyors')->check())
                    <div class="text-muted mt-1">Petugas Surveyor</div>
                @elseif(Auth::guard('surveyors_perusahaan')->check())
                    <div class="text-muted mt-1">Perusahaan Surveyor</div>
                @elseif(Auth::guard('adminstrator')->check())
                ...
                <div class="navi mt-2">
                    <a href="#" class="navi-item">
                        <span class="navi-link p-0 pb-2">
                            ...
                            @if(Auth::guard('traders')->check())
                                <span class="navi-text text-muted text-hover-primary">{{Auth::guard('traders')->user()->email}}</span>
                            @elseif(Auth::guard('surveyors')->check())
                                <span class="navi-text text-muted text-hover-primary">{{Auth::guard('surveyors')->user()->email}}</span>
                            @elseif(Auth::guard('surveyors_perusahaan')->check())
                                <span class="navi-text text-muted text-hover-primary">{{Auth::guard('surveyors_perusahaan')->user()->email}}</span>
                            @elseif(Auth::guard('adminstrator')->check())
                                <span class="navi-text text-muted text-hover-primary">{{Auth::guard('adminstrator')->user()->email}}</span>
                            @else
                    ...
                    <form method="post" action="{{route('logout')}}">
                        {{ csrf_field() }}
                        <button type="submit" class="btn btn-sm btn-light-primary font-weight-bolder py-2 px-5">Sign Out</a>
                    </form>
                </div>
            </div>
        </div>
