    ...
    <div class="subheader py-2 py-lg-6  subheader-transparent " id="kt_subheader">
        <div class=" container  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            ...
            <div class="d-flex align-items-center flex-wrap">
                @if($cek == null)
                <a href="{{route('traders.add_sk.bb')}}" class="btn btn-primary btn-sm">Tambah SK</a>
                @elseif($status_approve == 1)
                <!--- aktif dan diterima -->
                <!-- <a onclick="cekPenambahan()" class="btn btn-success btn-sm"> Penambahan SK</a> -->
                <a href="{{route('traders.perpanjang_sk.bb')}}" class="btn btn-warning btn-sm">Perpanjangan SK</a>
                                ...
                                <table class="table table-datatable table-custom" id="advancedDataTable">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            ?>
                                            <td>@if( $diff<= 30 && $diff>= 0)
                                                    <b style="color:#ffe20a;">{{tgl_indo($rs->masa_berlaku)}}
                                                    </b>
                                                    @elseif($diff <= 0 ) <b style="color:#e70309">
                                                        {{tgl_indo($rs->masa_berlaku)}}</b>
                                                        @else
                                                        {{tgl_indo($rs->masa_berlaku)}}
                                                        @endif
                                            </td>
