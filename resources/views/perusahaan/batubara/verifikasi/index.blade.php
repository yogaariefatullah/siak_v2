                    ...
                    <div class="card card-custom gutter-b">
                        <div class="card-header bg-color-navy">
                            <div class="card-title">
                                <h3 class="card-label text-white">Konfirmasi Pembelian Batubara</h3>
                            </div>
...
<script>
    $(function() {
        ...
        var table = $('#users-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('traders.verifikasi.batubara.data_pemasaran_perusahaan')}}",
            columns: [{
                    data: "no",
                    name: "no",
                    width: "3%"
        ...
        var table2 = $('#trader-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('traders.verifikasi.batubara.data_pemasaran_trader')}}",
            columns: [{
                    data: "no",
                    name: "no",
                    width: "3%"
        ...
        $(document.body).on('click', '.js-submit-confirm2', function(event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.post(
                        "{{ url('traders/verifikasi/batubara/reject/').'/'}}" + $('#id_pemasaran').val(), {
                            _token: '{{ csrf_token() }}',
                            alasan: result.value
                        },
                        function(res) {
        ...
        $(document.body).on('click', '.js-submit-confirm-tolak_trader', function(event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.post(
                        "{{ url('traders/verifikasi/batubara/tolak_trader/').'/'}}" + $('#id_pemasaran').val(), {
                            _token: '{{ csrf_token() }}',
                            alasan: result.value
                        },
                        function(res) {
    ...
    function showDetail(id_pemasaran) {
        $('.body-content').show();
        $.ajax({
            url: "{{route('traders.verifikasi.batubara.detail_pemasaran_perusahaan')}}",
            data: {
    ...
    function showtrader(id_pemasaran) {
        $('.body-content').show();
        $.ajax({
            url: "{{route('traders.verifikasi.batubara.detail_pemasaran_trader')}}",
            data: {
                id_pemasaran: id_pemasaran
            },
            beforeSend: function() {
