...
<div class="modal fade bs-example-modal-lg" id="modal_edit" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-xl ">
        <div class="modal-content">
            <div class="modal-header" style="background: #1aa3ff">
                <h2 class="modal-title" id="myModalLabel" style="color: whitesmoke;">Ubah Pemasaran</h2>
            </div>
...
<script>
    ...
    $(function() {
        var table = $('#advancedDataTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('traders.pemasaran.batubara.data_pemasaran')}}",
            columns: [{
                    data: "date",
                    name: "date"
                },
    ...
    function showDetail(id_pemasaran) {
        $('.body-content').show();
        $.ajax({
            url: "{{route('traders.pemasaran.batubara.detail_pemasaran')}}",
            data: {
    ...
    function add_pemasaran(data) {
        ...
        $('.body-contents').show();
        $.ajax({
            url: "{{route('traders.pemasaran.batubara.add_pemasaran')}}",
            data: {
                id_perusahaan: data
            },
            beforeSend: function() {
    ...
    function edit(id_pemasaran) {
        $('#modal_edit').modal('show');
        $.ajax({
            url: "{{route('traders.pemasaran.batubara.ubah_pemasaran')}}",
            data: {
                id_pemasaran: id_pemasaran
            },
            beforeSend: function() {
