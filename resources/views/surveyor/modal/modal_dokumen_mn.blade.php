    ...
    <div class="col-md-6">
        <table class="table table-borderless table-responsive-lg">
            ...
            <tr>
                ...
                <td>:</td>
                @if($tipe_dokumen == 'invoice')
                <td>
                    <p><a target="_blank" rel="noopener noreferrer" href="{{$dok_shipping}}">Dokumen Invoice</a></p>
                </td>
                @else
                <td>
                    <p><a target="_blank" rel="noopener noreferrer" href="{{$dok_shipping}}">Dokumen Bukti Bayar</a></p>
                </td>
                @endif
            </tr>
        </table>
...
<div class="row">
    ...
    <div class="col-md-3">
        ...
        <form id="btnVerif" action="{{$url}}" method="post">
            {{csrf_field()}}
...
<script>
    function confirm() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{route('surveyors.dokumen.mn.reject')}}",
                    type: 'POST',
                    data: {
                        _token: '{{csrf_token()}}',
